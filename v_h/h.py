...
def register(dt):
    try:
        ...
        for i in range(5):
            ...
            logging.warning(ip_text)
            logging.warning(city_text)
            telegram.send_doc(caption=f'{name}{index}слот{t}Hfan{user} Не прогрузился язык или дата{ip_text}-{city_text}', html=driver.page_source)
            raise RuntimeError(f'Не прогрузился язык или дата {name}-{index}слот{t}Hfan{user}')
        ...
        logging.warning(f'Ввод имя: {name}{index}слот{t}Hfan{user}')
            ...
            if f.is_element_displayed('//span[text()="Свободно"]'):
                ...
                if f.is_element_displayed(
                    ...
                    if f.is_element_displayed('//button[text()="Хорошо"]'):
                        for i in range(10):
                            try:
                                f.click_on('//div[contains(text(), "Сообщаем вам")]')
        ...
        try:
            ...
            telegram.send_message(f'В неизвестная ошибка.{index}слот{t}Hfan{user} {str(e)}')
